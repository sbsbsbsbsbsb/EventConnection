        ...
        public override IValidationMessage Validate(PropertyInfo property, IValidatable sender)
        {
            ...
            if (!string.IsNullOrEmpty(ComparisonProperty))
            {
                ...
                if (secondaryPropertyValue != null && !(secondaryPropertyValue is string))
                {
                    ...
                    {
                        LessThanValue = output;
                    }
                } 
                else if (secondaryPropertyValue is string)
                {
                    LessThanValue = secondaryPropertyValue.ToString().Length;
                }
            }
            ...
            if (value is string)
            {
                validationResult = value.ToString().Length > LessThanValue ? validationMessage : null;
            }
            return RunInterceptedValidation(sender, property, validationResult);
        }
    }
}
