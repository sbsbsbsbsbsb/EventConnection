...
namespace Tools.ValidationEngine.Models.ValidationRules
{
    ...
    public abstract class ValidationAttribute : Attribute, IValidationRule
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(InterceptionDelegate))
            {
                return message;
            }
            ...
            {
                DelegateName = InterceptionDelegate,
        ...
        protected object GetComparisonValue(object sender, string alternateProperty)
        {
            if (!string.IsNullOrEmpty(alternateProperty))
            {
                ...
                try
                {
                    comparisonProperty = sender.GetType().GetTypeInfo().GetDeclaredProperty(pathToProperty[0]);
                }
                catch (Exception)
                {
                    throw;
                ...
                else if (pathToProperty.Length > 1)
                {
                    ...
                    for (int index = 1; index < pathToProperty.Length; index++)
                    {
                        try
                        {
                            ...
                            comparisonProperty = childSender.GetType().GetTypeInfo().GetDeclaredProperty(pathToProperty[index]);
                        }
                        catch (Exception)
                        {
                            throw;
